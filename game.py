...
def best_of_generation(birds):
    ...
    best = None
    for bird in birds:
        if _fitness < bird.fitness:
            _fitness = bird.fitness
            best = bird

        if _score < bird.score:
            _score = bird.score

    logger(best)

    return best, _score, _fitness
    ...
    while running:
        ...
        if training:
            screen.blit(font.render(
                f'Generation: {generation}', True, (0, 0, 0)), dest=(10, 20))
            screen.blit(font.render(
                f'Population: {population}', True, (0, 0, 0)), dest=(10, 35))
            screen.blit(font.render(
                f'Bird number: {bird_number}', True, (0, 0, 0)), dest=(10, 50))
            screen.blit(font.render(
                f'The best score: {best_score}', True, (0, 0, 0)), dest=(10, 65))

        screen.blit(font.render(
            f'Score: {bird.sprite.score}', True, (0, 0, 0)), dest=(10, 5))
