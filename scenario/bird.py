...
class Bird(pygame.sprite.Sprite):
    def __init__(self, screen, x, y, brain=None):
        ...
        self.images = [
            pygame.image.load(
                './resources/bluebird-upflap.png').convert_alpha(),
            pygame.image.load(
                './resources/bluebird-midflap.png').convert_alpha(),
            pygame.image.load(
                './resources/bluebird-downflap.png').convert_alpha()
        ]
    ...
    def think(self, screen, pipes):
        ...
        pygame.draw.line(screen, (255, 0, 0),
                         (bird_x, bird_y), (target_x, target_y))
    ...
    def create_generation(screen, n, copy_bird=None):
        ...
        for i in range(n):
            """
            half = ceil((screen.get_height() / 2) / 2)
            x = np.random.randint(50, 250)
            y = np.random.randint(half, screen.get_height() - half)
            """
            ...
            bird = Bird(screen, screen.get_width() / 3,
                        screen.get_height() / 2, brain=brain)
